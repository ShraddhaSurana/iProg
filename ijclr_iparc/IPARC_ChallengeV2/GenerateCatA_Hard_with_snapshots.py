...
def write_snapshots_json_CatA_Hard(data, list_se_idx, color_rule, param, fname):  # <<< ADDED
    """
    Writes snapshot after all operations per band. One snapshot per example.
    """  # <<< ADDED
    snapshot_data = []  # <<< ADDED
    for (inp, _) in data:  # <<< ADDED
        img = Process(np.array(inp), num_colors=param['no_colors'])  # <<< ADDED
        snapshots = {}  # <<< ADDED
        for color, se_list in zip(range(param['no_colors']), list_se_idx):  # <<< ADDED
            band = img[:, :, color]  # <<< ADDED
            for idx in se_list:  # <<< ADDED
                band = binary_dilation(band, list_se_3x3[idx])  # <<< ADDED
            for idx in se_list:  # <<< ADDED
                band = binary_erosion(band, list_se_3x3[idx])  # <<< ADDED
            snapshots[f'band_{color+1}'] = band.astype(int).tolist()  # <<< ADDED
        snapshot_data.append(snapshots)  # <<< ADDED

    with open(fname, "w") as f:  # <<< ADDED
        json.dump(snapshot_data, f)  # <<< ADDED
...
def generate_100_tasks_CatA_Hard(seed, **param):
    ...
    """
    np.random.seed(seed)
    os.makedirs("./Dataset/CatA_Hard_snapshots", exist_ok=True)
    for task_no in range(100):
        data, list_se_idx, color_rule = generate_one_task_CatA_Hard(**param)
        fname = './Dataset/CatA_Hard_snapshots/Task{:03d}.json'.format(task_no)
        write_dict_json_CatA_Hard(data, fname)

        fname = './Dataset/CatA_Hard_snapshots/Task{:03d}_soln.txt'.format(task_no)
        write_solution_CatA_Hard(list_se_idx, color_rule, fname)

        fname = './Dataset/CatA_Hard_snapshots/Task{:03d}_soln.json'.format(task_no)
        write_solution_CatA_Hard_json(list_se_idx, color_rule, fname)

        fname = './Dataset/CatA_Hard/Task{:03d}_snapshot.json'.format(task_no)  # <<< ADDED
        write_snapshots_json_CatA_Hard(data, list_se_idx, color_rule, param, fname)  # <<< ADDED
